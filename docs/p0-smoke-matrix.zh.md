...
| PG-01 | PostgreSQL 初始化 | 空 PostgreSQL | 执行全部版本化 migration 两次 | 首次成功；第二次无漂移 | P1 执行 |
| PG-02 | PostgreSQL 事务 | PG-01 | 模拟提交成功但响应丢失后重试 | 相同幂等键只产生一份结果 | P1 执行 |
| PG-03 | PostgreSQL 并发 | PG-01 | 并发追加同一 session event | seq 唯一且连续，CAS 冲突可识别 | P1 执行 |
| CACHE-01 | Redis 恢复 | PostgreSQL + Redis | 写会话、清空 Surface cache、再次读取 | 从 Event Log 重建且内容一致 | P1 执行 |
...
| AUTH-01 | Scope | 两个测试用户 | 用户 B 请求用户 A 的 session/document | 404/拒绝且有审计事件 | P1 执行 |
